          ...
          <el-table-column label="操作" width="150">
            <template slot-scope="scope">
              ...
              <el-button
                ...
                icon="el-icon-search"
                v-if="
                  checkButton('001011001005') || checkButton('001011002005')
                "
...
<script>
  import ProductQuaDetailList from "@/components/qua/productquadetaillist";
  import * as api from "@/api/requestpath";

  export default {
  components: { ProductQuaDetailList },
  props: {
  ...
  methods: {
    ...
    fetchData() {
      ...
      let params = {
        ...
        quaCode: _this.orgInfoDetail.quaCode,
        quaType: 2
      };
      api
        .searchproductqualist(params)
        .then(rs => {
          if (rs.result === 200) {
            _this.loading = false;
        ...
        .catch(err => {
          _this.loading = false;
          console.log("api searchproductqualist error", err);
        });
    },
    ...
    handleSearchQuaDetail: function(quaInfoDetail) {
      let _this = this;
      console.log("企业资质机构信息" + JSON.stringify(quaInfoDetail));
